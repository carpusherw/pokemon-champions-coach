...
- `knowledge-bundle/` -- upload these 4 files under the Gem's Knowledge
  panel. Generated from `references/` (plus the two skill-bundled reference
...
3. Under Knowledge, upload all 3 files from `knowledge-bundle/`.
...
2. Run `python3 agents/gemini/pokemon-champions-coach/scripts/build_knowledge_bundle.py`
   ...
   that changes `references/`, `speed-mechanics.md`, or `archetypes.md`
   without also regenerating this bundle, so this step can't silently get
   skipped.
3. In Gem Manager, remove the 4 old Knowledge files and upload the new
   ones (there's no Gems API to automate this upload).
4. If `agents/claude/.../move-order-coach/SKILL.md`, `.../team-builder/SKILL.md`,
   or `.../skill-retro/SKILL.md` changed in a way that affects behavior (not
   just data), re-read it and hand-update `gem-instructions.md` too --
   that file is a judgment-based condensation, not something CI can
   diff-check for correctness the way it checks the knowledge bundle.
...
- **10-file Knowledge cap.** The bundle only ships species legal in the
  *current* regulation, not full historical rosters — that's a deliberate
  cut, not a bug, to leave headroom under the cap (4 of 10 files used as of
  `team-builder`'s addition).
