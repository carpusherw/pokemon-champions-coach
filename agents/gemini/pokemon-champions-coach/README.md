...
This folder packages the plugin's two portable skills for that surface.
...
- `knowledge-bundle/` -- upload these 3 files under the Gem's Knowledge
  panel. Generated from `references/` by `scripts/build_knowledge_bundle.py`;
...
2. Run `python3 agents/gemini/pokemon-champions-coach/scripts/build_knowledge_bundle.py`
   ...
   that changes `references/` or `speed-mechanics.md` without also
   regenerating this bundle, so this step can't silently get skipped.
3. In Gem Manager, remove the 3 old Knowledge files and upload the new
   ones (there's no Gems API to automate this upload).
4. If `agents/claude/.../move-order-coach/SKILL.md` or
...
- **10-file Knowledge cap.** The bundle only ships species legal in the
  *current* regulation, not full historical rosters — that's a deliberate
  cut, not a bug, to leave headroom under the cap.
