...
saved-teams/                    Best-effort local cache of saved teams (git-ignored), read/written
└── <slug>.yaml                 by team-recorder -- a convenience, not the primary save mechanism
                                 (see team-recorder's SKILL.md: the portable "team card" it prints
                                 in-chat is what actually survives across devices/ephemeral sessions)
...
- **team-recorder** — Turns a finished team into a portable "team card" (a copy-pasteable block) so it doesn't have to be rebuilt in a later session or against a new matchup; also best-effort caches it to `saved-teams/` when the local filesystem happens to persist, but the card itself — not the file — is what works across devices and ephemeral sessions. Works standalone (a team-builder run isn't required): give it a screenshot of the in-game team screen, a pasted team sheet, or a plain description. Both team-builder skills hand off to it, and `move-order-coach` reads saved teams back by name.
